...
func (m *DB) getRawTables() map[string]pgTable {
...
func (m *DB) getColumns(table string) (columns []pgColumn, err error) {
...
func (m *DB) getIndexes() (indexes map[string][]pgIndex, err error) {
...
func (m *DB) getForeignKeys() (foreignKeys map[string][]pgForeignKey, err error) {
	dbName := m.databaseName
	fkMap := make(map[string]pgForeignKey)
...
func (m *DB) descriptionFromRawTables(rawTables map[string]pgTable) db.DatabaseDescription {
...
func (m *DB) getManyManyDescription(t pgTable) (mm db.ManyManyDescription, ok bool) {
	td := m.getTableDescription(t)
	if len(td.Columns) != 2 {
