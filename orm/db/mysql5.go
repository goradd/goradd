...
// However, it does not work as expected. It appears to convert all times to this timezone before sending them
...
//
// Set the ParseTime configuration paramater to TRUE so that the driver will parse the times. Otherwise, we
// can only assume that the database is in UTC time, since we will not get any timezone info from the server.
//
// Since the driver will return times in the timezone of the mysql server, our own sql driver will convert
// these to local time. This will mean that you can save data in local time, and you will receive data in local
// time in your app, but you need to be aware that when you view the data in the SQL, it will appear in whatever
